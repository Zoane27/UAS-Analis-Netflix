...
# 1. Pengenalan Dataset
    ...
    st.write(
        "Dataset yang digunakan dalam analisis ini berisi informasi tentang film dan acara TV yang tersedia di Netflix. "
        "Dataset ini mencakup berbagai atribut seperti judul, jenis konten (film atau acara TV), tahun rilis, genre, "
        "negara asal, serta informasi tambahan seperti deskripsi dan pemeran utama. "
        "Tujuan utama dari eksplorasi dataset ini adalah untuk memahami pola distribusi konten serta mengidentifikasi "
        "insight yang dapat diambil dari data yang tersedia."
    )
...
# 2. Statistik Dasar
    ...
    st.write(
        "Statistik deskriptif memberikan gambaran umum tentang distribusi data dalam dataset. "
        "Dengan melihat statistik seperti rata-rata, nilai minimum, maksimum, serta standar deviasi, "
        "kita dapat memahami pola dalam data dan mendeteksi kemungkinan anomali atau outlier. "
        "Analisis statistik ini berguna untuk mendapatkan insight awal sebelum melakukan eksplorasi lebih lanjut."
    )
    ...
    st.markdown(
        ...
        - **Mean (Rata-rata):** Nilai tengah dari data.
        - **Standard Deviation (Standar Deviasi):** Mengukur seberapa jauh data tersebar dari rata-rata.
        - **Min & Max:** Menunjukkan nilai terkecil dan terbesar dalam setiap kolom.
        - **Percentiles (25%, 50%, 75%):** Membantu memahami distribusi data, dengan 50% sebagai median.
...
# 3. Distribusi Tahun Rilis
    ...
    st.write(
        "Analisis distribusi tahun rilis membantu memahami tren produksi film dan acara TV di Netflix. "
        "Dengan melihat pola rilis dari tahun ke tahun, kita dapat mengetahui periode dengan produksi tertinggi "
        "dan bagaimana tren industri hiburan berkembang dari waktu ke waktu."
    )
    ...
    st.markdown(
        """
        - **Tahun dengan Rilis Terbanyak:** Tahun dengan jumlah film/show terbanyak dalam dataset.
        - **Tahun dengan Rilis Terendah:** Tahun dengan jumlah film/show paling sedikit dalam dataset.
        - **Tren Rilis:** Grafik garis menunjukkan bagaimana jumlah rilis berubah dari waktu ke waktu.
        - **Insight yang Dapat Diperoleh:** Kita dapat melihat apakah ada peningkatan atau penurunan produksi film/show di Netflix pada tahun-tahun tertentu.
        """
    )
...
# 4. Distribusi Rating
    ...
    st.write(
        "Rating digunakan untuk mengkategorikan film dan acara TV berdasarkan kelayakan usia serta konten yang terkandung di dalamnya. "
        "Analisis distribusi rating membantu memahami jenis konten yang paling dominan di Netflix dan bagaimana platform ini menargetkan berbagai segmen penonton."
    )
    ...
    rating_descriptions = {
        ...
        "TV-Y7-FV": "Varian TV-Y7 dengan kekerasan fantasi lebih banyak, sering ditemukan di animasi.",
        "TV-Y": "Aman untuk semua usia, ditujukan untuk anak-anak.",
        "TV-G": "Cocok untuk semua umur, tanpa unsur kekerasan, bahasa kasar, atau tema dewasa.",
    ...
    # Tambahan untuk menangani durasi film dalam menit (jika ada dalam dataset)
    for duration in df_filtered["rating"].unique():
        if "min" in str(duration):  
            rating_descriptions[str(duration)] = "Durasi film dalam menit."
    ...
    # Tampilkan data dalam tabel (full width)
...
# 5. Perbandingan Film vs TV Show
    ...
    st.write(
        "Netflix menyediakan berbagai jenis konten, termasuk film dan acara TV. "
        "Analisis ini bertujuan untuk melihat apakah platform lebih banyak menghadirkan film atau serial TV, "
        "serta memahami pola distribusi kontennya."
    )
...
# 6. 10 Negara dengan Film/Show Terbanyak
    ...
    st.write(
        "Netflix menawarkan konten dari berbagai negara di seluruh dunia. "
        "Analisis ini bertujuan untuk mengetahui negara mana yang paling banyak berkontribusi dalam koleksi film dan acara TV di platform ini. "
        "Dengan memahami distribusi ini, kita dapat melihat tren produksi konten global di Netflix."
    )
...
# 7. Jumlah Film/Show per Rating
    ...
    st.write(
        "Rating merupakan indikator penting dalam menentukan batas usia yang sesuai untuk suatu film atau acara TV. "
        "Analisis ini bertujuan untuk melihat distribusi film dan acara TV berdasarkan rating yang diberikan, "
        "sehingga kita bisa memahami pola klasifikasi konten di Netflix."
    )
...
# 8. Analisis Durasi Film
    ...
    st.write(
        "Durasi film merupakan faktor penting yang memengaruhi pengalaman menonton. "
        "Analisis ini bertujuan untuk memahami pola distribusi durasi film di Netflix, "
        "termasuk durasi rata-rata, film terpendek, dan film terpanjang yang tersedia dalam dataset."
    )
...
def analisis_geoanalisis(df):
    shapefile_path = r'C:\Users\ASUS TUF GAMING F15\Documents\TUGAS UAS PAK FAHRIAN FINAL\TUGAS UAS PAK FAHRIAN\ne_110m_admin_0_countries'
    ...
    st.write(
        "Analisis geoanalisis digunakan untuk memahami distribusi produksi film dan acara TV berdasarkan negara. "
        "Dengan menggunakan data geografis, kita dapat melihat bagaimana persebaran produksi konten di berbagai negara "
        "dan mengetahui negara mana yang memiliki jumlah produksi film/TV shows terbanyak."
    )
...
def analisis_klasterisasi(df):
    ...
    st.markdown("""
    Dalam analisis ini, kami menerapkan metode **klasterisasi K-Means** untuk mengelompokkan film berdasarkan **rating** dan **durasi**. 
    Tujuan dari klasterisasi ini adalah untuk mengidentifikasi pola umum dalam distribusi film serta menemukan hubungan antara rating dan durasi film dalam dataset.
    
    **Mengapa Klasterisasi?**  
    - Membantu mengelompokkan film berdasarkan kesamaan karakteristik.  
    - Menyediakan wawasan apakah film dengan rating tertentu cenderung memiliki durasi yang lebih panjang atau pendek.  
    - Dapat digunakan untuk memahami tren dalam industri film berdasarkan durasi dan rating.  
    """)
...
# 11. Film Details
    ...
    st.markdown("""
    Dalam bagian ini, Anda dapat melihat informasi lebih mendalam tentang film atau TV Show yang ada dalam dataset. Anda bisa memilih film berdasarkan filter yang disediakan untuk melihat detail terkait film tersebut.  
    Fungsi ini memungkinkan Anda untuk menjelajahi informasi seperti judul, tipe (film atau TV show), sutradara, pemeran, negara asal, tahun rilis, rating, durasi, kategori, dan deskripsi singkat film.
    
    **Cara Menggunakan:**
    - **Pilih Tipe:** Menentukan apakah Anda ingin melihat film atau TV show.
    - **Pilih Negara:** Menyaring film berdasarkan negara asal.
    - **Pilih Tahun Rilis:** Menyaring film berdasarkan tahun rilis.
    Setelah memilih filter, Anda dapat memilih film yang sesuai untuk melihat detail lebih lanjut.

    Jika filter yang diterapkan tidak menghasilkan film yang sesuai, Anda akan diberi peringatan bahwa tidak ada film yang ditemukan berdasarkan kriteria tersebut.
    """)
